...
// 		            dao.updateUser(id,address,email) 그냥 3개 선언해서 처리해도 됨
	...
	} else if(command.equals("userroleform")){// 나의 정보 조회
		...
		boolean isS=dao.updateUserRole(new UserDto(id,role));
// 		            dao.updateUser(id,address,email) 그냥 3개 선언해서 처리해도 됨
		if(isS){
			%>
			<script type="text/javascript">
				alert('수정완료');
				location.href="userAllList.jsp";
			</script>
			<%
		}else{
			%>
	...
	} else if(command.equals("getAllUserList")){// 나의 정보 조회
	...
	} else if(command.equals("getUserList")){// 나의 정보 조회
